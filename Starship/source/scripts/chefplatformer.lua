...
function Chef:init(x, y, gameManager)
    ...
    -- State Machine
    local chefImageTable = gfx.imagetable.new("/assets/images/chef-table-64-64")
    ...
    self:setCollideRect(22, 20, 26, 41)
    self:setTag(TAGS.Chef)

    -- Physics properties
    self.xVelocity = 0
    self.yVelocity = 0
    self.gravity = planetGrav
    self.maxSpeed = 5
    self.jumpVelocity = -8
    self.drag = 0.1
...
function Chef:update()
    ...
    if _G.paused == false then
        ...
        if pd.buttonJustReleased(pd.kButtonB) and playdate.buttonIsPressed(playdate.kButtonUp) then
...
function ShipTakeoff:init(x, y)
    ...
    if shipClass == "Taco" then
        shiptakeoffImageTable = gfx.imagetable.new("assets/images/tacotakeoff-table-400-240")
    elseif shipClass == "Burger" then
