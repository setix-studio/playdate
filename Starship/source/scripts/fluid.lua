...
function Fluid:touch(x, velocity)
    ...
    local vertex_index <const> = clamp(round((((x - self.bounds.x) / self.bounds.width) * (self.vertex_count - 1)) + 1),
        1, self.vertex_count)
...
function Fluid:updatePolygon()
    for i, vertex in pairs(self.vertices) do
        self.polygon:setPointAt(i, self.bounds.x + ((i - 1) * self.column_width),
...
function Fluid:getWaveBounds()
    return geometry.rect.new(self.bounds.x, (self.bounds.y + self.bounds.height) - self.wave_range.tallest,
        self.bounds.width, self.wave_range.tallest - self.wave_range.shortest)
end
